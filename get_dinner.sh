...
TODAY=$(date +%Y%m%d) 
NAMES=$(cat /BOB/data/$TODAY | grep "var DinnerListJson" | awk '{print substr($0,index($0,$4))}' | sed 's/\;//g' | /usr/local/bin/jq ".[]|.CfMenu_Food" | uniq |  sed 's/\"//g')
...
DATA=$(cat /BOB/data/$TODAY | grep "var DinnerListJson" | awk '{print substr($0,index($0,$4))}' | sed 's/\;//g' | /usr/local/bin/jq "[.[]| {index:.CfMenu_Name,title: .CfMenu_Food, text: [.MenuFoodList| .[] | .CfFood_Name]}]" |sed -e '/\"\",/d' | /usr/local/bin/jq 'unique'| /usr/local/bin/jq ". | map(.image_url = if (.index) == "'"'"B"'"'" then "'"'"${IMAGES[1]:-$NOIMAGE}"'"'" else "'"'"${IMAGES[0]:-$NOIMAGE}"'"'" end)" | tr '\n' ' ' | sed 's/",       "/\\n/g' | sed 's/\[//g' | sed 's/\]//g')
